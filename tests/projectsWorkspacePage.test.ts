...
describe('Projects workspace list UI', () => {
  ...
  it('renders recognizable product marks for typed external resources', () => {
    ...
    expect(markup).toContain(`data-testid="project-resource-date-chip:last-checked:${docs.id}"`)
    expect(markup).toContain('aria-label="Last checked')
    expect(markup).toContain('tabler-icon-calendar-check')
    expect(markup).toContain('hover:bg-surface-subtle-hover')
    expect(markup).toContain('tabler-icon-hexagon-filled')
    expect(markup).toContain('color-mix(in srgb')
    expect(markup).toContain('var(--project-icon-surface-strength)')
