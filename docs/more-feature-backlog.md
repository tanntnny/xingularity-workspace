...
> Product, architecture, safety, and workflow backlog derived from the current working tree on 2026-08-17. This is an application audit and planning document; it does not claim that any item below has been implemented.
...
The current page contract in [navigation.ts](../src/renderer/src/navigation.ts), [pageAvailability.ts](../src/renderer/src/platform/pageAvailability.ts), [AppSidebar.tsx](../src/renderer/src/components/AppSidebar.tsx), and [App.tsx](../src/renderer/src/App.tsx) exposes:
...
### Implemented or documented but not currently exposed as normal pages

The current codebase also contains backend contracts or documentation for capabilities that are not represented in the current AppPage union and sidebar:

* weekly plans have main-process services, IPC types, storage, and history support, but no normal renderer page

* agent chat and agent history have stores, main-process runtime methods, IPC handlers, and preload methods, but no normal renderer page

* the application documentation still describes Dashboard and Grid surfaces that are not in the current navigation contract

* [app-features.md](app-features.md), [README.md](../README.md), and parts of [app-pages-and-data.md](app-pages-and-data.md) still describe the legacy calendar/tasks.json layout while the current task store uses tasks/<task-id>.json

This may be intentional staging, but a user should not have to infer whether a feature is shipped, hidden, experimental, or backend-only.

### Current calendar truth

The calendar is a local task planner, not an external calendar integration:
...
* the renderer persists the task collection through settings/task APIs and renders month and week views

* the month view treats events as all-day date-range items; the week view derives timed entries when a task has a time

* CalendarDayView exists but is not mounted by the current App route

* filters cover project/non-project and tags; there is no provider, account, calendar, attendee, location, recurrence, timezone, external event ID, ETag, or sync cursor

* repository search found no Google Calendar, OAuth, iCal, CalDAV, provider adapter, or external-event implementation

The implication is that Google Calendar should be built on a calendar-domain contract, not added as a few provider fields on the current task object.
