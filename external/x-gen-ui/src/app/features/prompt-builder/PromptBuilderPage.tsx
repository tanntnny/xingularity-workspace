...
export function PromptBuilderPage() {
  ...
  const prompt = useMemo(
    () => buildPrompt({ artifactType, style, allowedComponents, topic }),
    [artifactType, style, allowedComponents, topic]
  )
    ...
    <div className="grid gap-3 xl:grid-cols-[420px_minmax(0,1fr)]">
      <section className="rounded-xl border border-white/70 bg-white/85 p-4 shadow-panel">
        ...
        <p className="mt-1 text-slate-600">
          Generate strict instructions for external chatbot browsers.
        </p>
        ...
        <select
          className="mt-2 w-full rounded-xl border border-slate-200 bg-white p-3 font-bold"
          value={style}
          onChange={(event) => setStyle(event.target.value as ArtifactStyle)}
        >
          {styles.map((item) => (
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            {PROMPT_COMPONENT_TYPES.map((component) => {
              ...
              return (
                <label
                  key={component}
                  className="flex cursor-pointer items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-bold"
                >
                  <input
                    type="checkbox"
                    checked={checked}
      ...
      <section className="rounded-xl border border-white/70 bg-slate-950 p-4 text-white shadow-panel">
        <div className="mb-3 flex items-center justify-between gap-2">
          ...
          <span className="rounded-lg bg-white/10 px-3 py-1 font-mono text-xs font-bold">
            {allowedComponents.length} components
          </span>
        </div>
        <textarea
          className="min-h-[650px] w-full resize-y rounded-xl border border-white/10 bg-black/30 p-4 font-mono text-sm leading-relaxed text-slate-100"
          value={prompt}
          onChange={() => undefined}
          readOnly
        />
