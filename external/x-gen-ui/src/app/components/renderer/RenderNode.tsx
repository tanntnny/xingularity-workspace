...
export function ArtifactRenderer({ artifact }: { artifact: Artifact }) {
  ...
  return (
    ...
    >
      <div
        className={cn('mb-4 h-2 rounded-md bg-gradient-to-r', accentClass[artifact.theme.accent])}
      />
      <div className="mb-4">
        ...
        {artifact.metadata.description ? (
          <p className={cn('mt-2', isDark ? 'text-slate-300' : 'text-slate-600')}>
            {artifact.metadata.description}
          </p>
        ) : null}
...
export function RenderNode({ node }: { node: UINode }) {
  ...
  return (
    <Renderer
      node={node as never}
      renderNode={(child, key) => <RenderNode key={key} node={child} />}
    />
  )
}
