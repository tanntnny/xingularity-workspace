...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  const tone =
    status === 'Valid' || status === 'Saved' ? 'good' : status === 'Invalid' ? 'bad' : 'warn'
      ...
      <header className="mx-auto mb-4 flex max-w-7xl flex-col gap-3 rounded-xl border border-white/70 bg-white/70 p-4 shadow-panel backdrop-blur md:flex-row md:items-center md:justify-between">
        <div>
          <p className="font-mono text-xs font-bold uppercase tracking-[0.28em] text-slate-500">
            Schema-first artifact renderer
          </p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="rounded-lg bg-slate-950 px-3 py-1.5 font-mono text-xs font-bold text-white">
            No LLM API
          </span>
        </div>
      </header>
