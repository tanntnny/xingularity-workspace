...
export function Editor({
  ...
  const editor = useCreateBlockNote({
    uploadFile: async (file: File) => {
      ...
      const imageUrl = await onPasteImage(file, `.${extension}`)
  ...
  useEffect(() => {
    // Convert relative markdown image paths to file:// URLs for initial load
    const displayValue =
      vaultRootPath && currentNotePath
        ? convertMarkdownImagesToFileUrls(value, vaultRootPath, currentNotePath)
        : value

    const initialBlocks = editor.tryParseMarkdownToBlocks(displayValue)
